...
const FROM = '521', TO = '522';
const publicDir = path.join(__dirname, 'public');
function processFile(f) {
    let c = fs.readFileSync(f, 'utf8'), o = c;
    c = c.replace(/\?v=521/g, `?v=${TO}`).replace(/CURRENT_VERSION\s*=\s*'521'/g, `CURRENT_VERSION = '${TO}'`)
         .replace(/Versión de la App: v521/g, `Versión de la App: v${TO}`).replace(/fitness-app-v521/g, `fitness-app-v${TO}`);
