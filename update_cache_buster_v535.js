...
const FROM = '534', TO = '535';
const publicDir = path.join(__dirname, 'public');
function processFile(f) {
    let c = fs.readFileSync(f, 'utf8'), o = c;
    c = c.replace(/\?v=534/g, `?v=${TO}`).replace(/CURRENT_VERSION\s*=\s*'534'/g, `CURRENT_VERSION = '${TO}'`)
         .replace(/Versión de la App: v534/g, `Versión de la App: v${TO}`).replace(/fitness-app-v534/g, `fitness-app-v${TO}`);
