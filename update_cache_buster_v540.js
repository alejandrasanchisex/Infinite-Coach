...
const FROM = '539', TO = '540';
const publicDir = path.join(__dirname, 'public');
function processFile(f) {
    let c = fs.readFileSync(f, 'utf8'), o = c;
    c = c.replace(/\?v=539/g, `?v=${TO}`).replace(/CURRENT_VERSION\s*=\s*'539'/g, `CURRENT_VERSION = '${TO}'`)
         .replace(/Versión de la App: v539/g, `Versión de la App: v${TO}`).replace(/fitness-app-v539/g, `fitness-app-v${TO}`);
