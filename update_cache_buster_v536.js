...
const FROM = '535', TO = '536';
const publicDir = path.join(__dirname, 'public');
function processFile(f) {
    let c = fs.readFileSync(f, 'utf8'), o = c;
    c = c.replace(/\?v=535/g, `?v=${TO}`).replace(/CURRENT_VERSION\s*=\s*'535'/g, `CURRENT_VERSION = '${TO}'`)
         .replace(/Versión de la App: v535/g, `Versión de la App: v${TO}`).replace(/fitness-app-v535/g, `fitness-app-v${TO}`);
