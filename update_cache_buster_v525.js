...
const FROM = '524', TO = '525';
const publicDir = path.join(__dirname, 'public');
function processFile(f) {
    let c = fs.readFileSync(f, 'utf8'), o = c;
    c = c.replace(/\?v=524/g, `?v=${TO}`).replace(/CURRENT_VERSION\s*=\s*'524'/g, `CURRENT_VERSION = '${TO}'`)
         .replace(/Versión de la App: v524/g, `Versión de la App: v${TO}`).replace(/fitness-app-v524/g, `fitness-app-v${TO}`);
