...
const FROM = '523', TO = '524';
const publicDir = path.join(__dirname, 'public');
function processFile(f) {
    let c = fs.readFileSync(f, 'utf8'), o = c;
    c = c.replace(/\?v=523/g, `?v=${TO}`).replace(/CURRENT_VERSION\s*=\s*'523'/g, `CURRENT_VERSION = '${TO}'`)
         .replace(/Versión de la App: v523/g, `Versión de la App: v${TO}`).replace(/fitness-app-v523/g, `fitness-app-v${TO}`);
