...
const FROM = '525', TO = '526';
const publicDir = path.join(__dirname, 'public');
function processFile(f) {
    let c = fs.readFileSync(f, 'utf8'), o = c;
    c = c.replace(/\?v=525/g, `?v=${TO}`).replace(/CURRENT_VERSION\s*=\s*'525'/g, `CURRENT_VERSION = '${TO}'`)
         .replace(/Versión de la App: v525/g, `Versión de la App: v${TO}`).replace(/fitness-app-v525/g, `fitness-app-v${TO}`);
