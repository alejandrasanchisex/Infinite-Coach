        ...
        if (key && key.indexOf('fitnessAppData_') === 0 && !key.endsWith('_backup') && clientId && !isTrainer) {
            try {
                ...
                if (data && data.clients) {
                    ...
                    if (data.habits) {
                        data.habits = data.habits.filter(h => String(h.clientId) === String(clientId));
                    }
                ...
                if (window.SupabaseService.client) {
                    ...
                    if (!scanError && profiles) {
                        ...
                        if (correctTid) {
                            ...
                            try {
                                const { data: dbProfiles } = await window.SupabaseService.client
                                    .from('trainer_profiles')
                                    .select('full_data')
                                    .eq('trainer_id', correctTid)
                                    .limit(1);
                                const trainerProfile = (dbProfiles && dbProfiles.length > 0) ? dbProfiles[0] : null;
...
const BrandConfig = {
  ...
  get: () => {
    ...
    if (isAlejandra) {
        ...
        defaultBrand = {
            name: 'Lucy Tundidor',
            logo: 'https://bieeydhacavxymoosasx.supabase.co/storage/v1/object/public/Media/lucy_logo_cropped.png?v=799',
            configured: true,
            colors: { 
                primary: '#816e61', 
                secondary: '#a79788', 
    ...
    if (isLucy && res && (res.name === 'Lucy Tundidor' || defaultBrand.name === 'Lucy Tundidor')) {
        ...
        if (!res.logo || res.logo === 'img/logo-infinite-coach.png' || res.logo.includes('1779724548154') || res.logo.includes('lucy_logo_v1.png') || !res.logo.includes('lucy_logo_cropped.png?v=799')) {
            res.logo = defaultBrand.logo;
            changed = true;
        }
        const expectedQuestions = [
