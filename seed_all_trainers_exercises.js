...
async function seedAllTrainersExercises() {
    console.log("🚀 Starting database-wide exercise seeding & shielding...");
    ...
    for (const profile of profiles) {
        ...
        // 1. Ensure muscleGroupsConfig exists
        if (!fullData.muscleGroupsConfig) {
            fullData.muscleGroupsConfig = {
                groups: targetGroups,
                exercises: {}
            };
            modified = true;
        } else {
            const currentGroups = fullData.muscleGroupsConfig.groups || [];
            if (JSON.stringify(currentGroups) !== JSON.stringify(targetGroups)) {
                fullData.muscleGroupsConfig.groups = targetGroups;
                modified = true;
            }
        }
        
        const mgConfig = fullData.muscleGroupsConfig;
        if (!mgConfig.exercises) {
            mgConfig.exercises = {};
            modified = true;
        }
        
        // 2. Ensure all PRO_EXERCISES exist in muscleGroupsConfig.exercises map
        ...
        // 3. Ensure all PRO_EXERCISES are present in profile's media library
        if (!fullData.media) {
            fullData.media = [];
            modified = true;
        }
        ...
        for (const group in PRO_EXERCISES) {
            PRO_EXERCISES[group].forEach(proEx => {
                const mediaIndex = fullData.media.findIndex(m => 
                    m.category === 'exercise' && 
                ...
                if (mediaIndex === -1) {
                    ...
                    const mediaItem = fullData.media[mediaIndex];
                    if (!mediaItem.url || mediaItem.isSystem || mediaItem.muscleGroup !== group) {
                        mediaItem.url = proEx.videoUrl;
                        mediaItem.muscleGroup = group;
                        mediaItem.isSystem = true;
                        modified = true;
                    }
                }
            });
        ...
        if (modified) {
            console.log(`  💾 Seeding exercises into Supabase for trainer ${profile.trainer_id}...`);
            ...
            if (updateError) {
                ...
                console.log(`  ✅ Successfully updated trainer ${profile.trainer_id}!`);
            }
        } else {
            console.log(`  ℹ️ Trainer ${profile.trainer_id} is already fully seeded.`);
        }
    }
    
    console.log("🎉 Seeding & Shielding of system-wide exercises completed successfully!");
}

seedAllTrainersExercises();
