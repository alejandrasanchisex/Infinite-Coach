...
$outputDir = "c:\Users\usuario\Desktop\Proyectos\App Fitness\public\img"

foreach ($key in $images.Keys) {
    $prompt = $images[$key]
    $encodedPrompt = [System.Web.HttpUtility]::UrlEncode($prompt)
    $url = "$baseUrl$encodedPrompt?width=800&height=800&nologo=true"
    $filepath = Join-Path -Path $outputDir -ChildPath $key
    
    Write-Host "Downloading $key..."
    # -L follows redirects, -s is silent
    curl.exe -s -L -o "$filepath" "$url"
