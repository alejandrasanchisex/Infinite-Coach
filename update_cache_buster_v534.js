...
const FROM = '533', TO = '534';
const publicDir = path.join(__dirname, 'public');
function processFile(f) {
    let c = fs.readFileSync(f, 'utf8'), o = c;
    c = c.replace(/\?v=533/g, `?v=${TO}`).replace(/CURRENT_VERSION\s*=\s*'533'/g, `CURRENT_VERSION = '${TO}'`)
         .replace(/Versión de la App: v533/g, `Versión de la App: v${TO}`).replace(/fitness-app-v533/g, `fitness-app-v${TO}`);
