...
const FROM = '538', TO = '539';
const publicDir = path.join(__dirname, 'public');
function processFile(f) {
    let c = fs.readFileSync(f, 'utf8'), o = c;
    c = c.replace(/\?v=538/g, `?v=${TO}`).replace(/CURRENT_VERSION\s*=\s*'538'/g, `CURRENT_VERSION = '${TO}'`)
         .replace(/Versión de la App: v538/g, `Versión de la App: v${TO}`).replace(/fitness-app-v538/g, `fitness-app-v${TO}`);
