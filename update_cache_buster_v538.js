...
const FROM = '537', TO = '538';
const publicDir = path.join(__dirname, 'public');
function processFile(f) {
    let c = fs.readFileSync(f, 'utf8'), o = c;
    c = c.replace(/\?v=537/g, `?v=${TO}`).replace(/CURRENT_VERSION\s*=\s*'537'/g, `CURRENT_VERSION = '${TO}'`)
         .replace(/Versión de la App: v537/g, `Versión de la App: v${TO}`).replace(/fitness-app-v537/g, `fitness-app-v${TO}`);
