...
const FROM = '536', TO = '537';
const publicDir = path.join(__dirname, 'public');
function processFile(f) {
    let c = fs.readFileSync(f, 'utf8'), o = c;
    c = c.replace(/\?v=536/g, `?v=${TO}`).replace(/CURRENT_VERSION\s*=\s*'536'/g, `CURRENT_VERSION = '${TO}'`)
         .replace(/Versión de la App: v536/g, `Versión de la App: v${TO}`).replace(/fitness-app-v536/g, `fitness-app-v${TO}`);
